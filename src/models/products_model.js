...
const productsModel = {
  ...
  getAll: () => {
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT products.id, products.name, products.image, products.price, products.price, products.description, products.stock, products.discount, category.name AS category, products.delivery_days, products.delivery_time FROM products INNER JOIN category ON products.category_id=category.id `,
        (err, result) => {
          if (err) {
            reject(err);
          } else {
